...
def resource_path(relative_path: str) -> str:
    """
    開発時: カレントディレクトリ基準
    EXE時: PyInstaller が展開する一時フォルダ (_MEIPASS) 基準
    でファイルパスを返すユーティリティ関数
    """
...
def page_composite_tolerance():
    ...
    st.write(
        ...
        **複合位置度（Composite Position Tolerance）** は，
        ...
        - 下段：パターン内部のピッチ・姿勢など（しばしばデータムが減る）
...
def page_composite_position_random():
    ...
    - 下段公差域：半径 2
    - 穴中心間距離： 30
    - 上段の指示：三平面データム系に対して、Φ5の円筒公差域に入っていることを要求
    - 下段の指示：4つの円筒(Φ2)はデータムAに対して垂直であり、円筒の軸間の距離は30を要求
...
def page_combined_tolerance():
    ...
    # --- 図面イメージ：タイトル直下に小さく表示 ---
    IMAGE_COMBINED = resource_path("images/combined_tolerance.png")
